const mongoose = require('mongoose')

const User = require('../../model/User')
const Zone = require('../../model/Zone')
const Lead = require('../../model/Leads')
const FollowUp = require('../../model/FollowUp')
const Country = require('../../model/Country')
const AppConfig = require('../../model/AppConfig')

const { successResponse, errorResponse } = require('../../util/response')

const getLeadConfigLookupStages = () => [
  {
    $lookup: {
      from: 'app_config',
      let: {
        statusId: '$lead_status_id',
        sourceId: '$source_id',
        solutionId: '$solution_id'
      },
      pipeline: [
        {
          $match: {
            type: 'leads_data'
          }
        },
        {
          $project: {
            leads_status_data: 1,
            leads_source_data: 1,
            solution_data: 1
          }
        },
        {
          $addFields: {
            status: {
              $arrayElemAt: [
                {
                  $filter: {
                    input: '$leads_status_data',
                    as: 'item',
                    cond: { $eq: ['$$item._id', '$$statusId'] }
                  }
                },
                0
              ]
            },
            source: {
              $arrayElemAt: [
                {
                  $filter: {
                    input: '$leads_source_data',
                    as: 'item',
                    cond: { $eq: ['$$item._id', '$$sourceId'] }
                  }
                },
                0
              ]
            },
            solution: {
              $arrayElemAt: [
                {
                  $filter: {
                    input: '$solution_data',
                    as: 'item',
                    cond: { $eq: ['$$item._id', '$$solutionId'] }
                  }
                },
                0
              ]
            }
          }
        },
        {
          $project: {
            _id: 0,
            status: 1,
            source: 1,
            solution: 1
          }
        }
      ],
      as: 'configData'
    }
  },
  {
    $addFields: {
      lead_status: {
        status: { $arrayElemAt: ['$configData.status', 0] }
      },
      source: { $arrayElemAt: ['$configData.source', 0] },
      solution: { $arrayElemAt: ['$configData.solution', 0] }
    }
  },
  {
    $project: {
      configData: 0
    }
  }
]

// Reusable pipeline stages for populating follow-ups and their configurations
const getFollowUpLookupStages = () => [
  {
    $lookup: {
      from: 'follow_up',
      let: { leadId: '$_id' },
      pipeline: [
        {
          $match: {
            $expr: { $eq: ['$lead_id', '$$leadId'] }
          }
        },
        {
          $sort: { created_at: -1 }
        }
      ],
      as: 'followUp'
    }
  },
  {
    $unwind: {
      path: '$followUp',
      preserveNullAndEmptyArrays: true
    }
  },
  {
    $addFields: {
      followUp: {
        $cond: [{ $ifNull: ['$followUp._id', false] }, '$followUp', null]
      }
    }
  },
  {
    $lookup: {
      from: 'app_config',
      let: {
        statusId: '$followUp.status',
        priorityId: '$followUp.priority',
        typeId: '$followUp.follow_up_type'
      },
      pipeline: [
        {
          $match: { type: 'follow_up_data' }
        },
        {
          $project: {
            follow_up_status_data: 1,
            follow_up_priority_data: 1,
            follow_up_type_data: 1
          }
        },
        {
          $addFields: {
            status: {
              $arrayElemAt: [
                {
                  $filter: {
                    input: '$follow_up_status_data',
                    as: 'item',
                    cond: { $eq: ['$$item._id', '$$statusId'] }
                  }
                },
                0
              ]
            },
            priority: {
              $arrayElemAt: [
                {
                  $filter: {
                    input: '$follow_up_priority_data',
                    as: 'item',
                    cond: { $eq: ['$$item._id', '$$priorityId'] }
                  }
                },
                0
              ]
            },
            follow_up_type: {
              $arrayElemAt: [
                {
                  $filter: {
                    input: '$follow_up_type_data',
                    as: 'item',
                    cond: { $eq: ['$$item._id', '$$typeId'] }
                  }
                },
                0
              ]
            }
          }
        },
        {
          $project: {
            _id: 0,
            status: 1,
            priority: 1,
            follow_up_type: 1
          }
        }
      ],
      as: 'followUpConfig'
    }
  },
  {
    $addFields: {
      'followUp.status_data': { $arrayElemAt: ['$followUpConfig.status', 0] },
      'followUp.priority_data': {
        $arrayElemAt: ['$followUpConfig.priority', 0]
      },
      'followUp.type_data': {
        $arrayElemAt: ['$followUpConfig.follow_up_type', 0]
      }
    }
  },
  {
    $project: {
      followUpConfig: 0
    }
  },
  {
    $group: {
      _id: '$_id',
      doc: { $first: '$$ROOT' },
      followUps: {
        $push: {
          $cond: [
            { $ifNull: ['$followUp._id', false] },
            '$followUp',
            '$$REMOVE'
          ]
        }
      }
    }
  },
  {
    $addFields: {
      'doc.followUp': { $ifNull: ['$followUps', []] }
    }
  },
  {
    $replaceRoot: { newRoot: '$doc' }
  }
]

exports.getLeadAPIController = async (req, res, next) => {
  try {
    const userId = req?.userId
    const { type } = req?.params
    let finalData = []

    const userObjectId = mongoose.Types.ObjectId.createFromHexString(userId)

    if (type === 'kanban') {
      const data = await AppConfig.aggregate([
        {
          $match: {
            type: 'leads_data'
          }
        },
        {
          $unwind: '$leads_status_data'
        },
        {
          $lookup: {
            from: 'leads',
            let: {
              statusId: '$leads_status_data._id',
              userId: userObjectId
            },
            pipeline: [
              {
                $match: {
                  $expr: {
                    $and: [
                      { $eq: ['$lead_status_id', '$$statusId'] },
                      { $eq: ['$created_by', '$$userId'] }
                    ]
                  }
                }
              },
              ...getFollowUpLookupStages(),
              ...getLeadConfigLookupStages()
            ],
            as: 'leads'
          }
        },
        {
          $project: {
            _id: 0,
            status: '$leads_status_data',
            leads: 1
          }
        }
      ])

      finalData = data
    } else {
      const leads = await Lead.aggregate([
        {
          $match: {
            created_by: userObjectId
          }
        },
        ...getFollowUpLookupStages(),
        ...getLeadConfigLookupStages(),
        {
          $lookup: {
            from: 'app_config',
            let: { statusId: '$lead_status_id' },
            pipeline: [
              { $match: { type: 'leads_data' } },
              { $unwind: '$leads_status_data' },
              {
                $match: {
                  $expr: { $eq: ['$leads_status_data._id', '$$statusId'] }
                }
              },
              {
                $project: {
                  _id: 0,
                  status: '$leads_status_data'
                }
              }
            ],
            as: 'lead_status'
          }
        },
        {
          $unwind: {
            path: '$lead_status',
            preserveNullAndEmptyArrays: true
          }
        }
      ])

      finalData = leads
    }

    return successResponse(res, 'Lead fetched successfully', finalData)
  } catch (error) {
    next(error)
  }
}

exports.getCreateLeadAPIController = async (req, res, next) => {
  try {
    const userId = req?.userId

    const user_curr = await User.findById(userId)
    const masterId = user_curr?.created_by

    const country = await Country.findOne({
      _id: '6821e073c67f35e8ab742e21'
    })
    const states = country?.states || []

    const zones = await Zone.find({
      created_by: masterId
    })

    const branchData = zones.flatMap(z =>
      (z.region || []).flatMap(r => r.branch || [])
    )

    const appConfig = await AppConfig.findOne({
      type: 'leads_data'
    })

    const sourceData = appConfig?.leads_source_data || []
    const statusData = appConfig?.leads_status_data || []
    const solutionData = appConfig?.solution_data || []

    const appFollowUpConfig = await AppConfig.findOne({
      type: 'follow_up_data'
    })

    const leadAppConfig = await AppConfig.findOne({
      type: 'leads_data'
    })

    const followUpType = appFollowUpConfig?.follow_up_type_data || []
    const followUpStatus = appFollowUpConfig.follow_up_status_data || []
    const followUpPriority = appFollowUpConfig.follow_up_priority_data || []
    const leadStatusData = leadAppConfig?.leads_status_data || []

    const finalData = {
      states,
      followUpType,
      followUpStatus,
      followUpPriority,
      statusData,
      sourceData,
      branchData,
      solutionData
    }

    return successResponse(
      res,
      'Leads create data fetched successfully',
      finalData
    )
  } catch (error) {
    next(error)
  }
}

const convertVar = val => {
  return val === '' ? undefined : val
}

exports.postLeadController = async (req, res, next) => {
  try {
    const userId = req?.userId

    const {
      company_name,
      solution_id,
      branch_id,
      address,
      average_monthly_consumption,
      city_id,
      email,
      lead_status_id,
      name,
      phone,
      pincode,
      sanctioned_load,
      source_id,
      state_id,
      allow_followup,
      follow_up_date,
      follow_up_type,
      status,
      priority,
      reminder_before,
      next_follow_up_date,
      notes
    } = req?.body

    const lead = new Lead({
      name,
      address,
      company_name,
      solution_id: convertVar(solution_id),
      pincode,
      email,
      phone,
      average_monthly_consumption,
      branch_id,
      city_id,
      state_id,
      lead_status_id,
      is_converted: false,
      source_id: convertVar(source_id),
      sanctioned_load,
      assigned_user_id: userId,
      created_by: userId,
      created_at: Date.now()
    })

    await lead.save()

    const follow_up = new FollowUp({
      lead_data: {
        lead_status_id,
        company_name: lead?.company_name,
        name: lead?.name,
        email: lead?.email,
        phone: lead?.phone,
        state_id: lead?.state_id,
        city_id: lead?.city_id,
        address: lead?.address,
        pincode: lead?.pincode
      },
      lead_id: lead?._id,
      follow_up_date,
      follow_up_type,
      next_follow_up_date,
      notes,
      priority,
      reminder_before,
      status,
      assigned_to: userId,
      created_by: userId
    })

    await follow_up.save()

    const isConverted = lead_status_id == '69d5dfb78c890e742280d9cb'

    await Lead.findByIdAndUpdate(lead?._id, {
      lead_status_id,
      is_converted: isConverted
    })

    return successResponse(res, 'Leads saved successfully')
  } catch (error) {
    next(error)
  }
}

exports.putLeadAPIController = async (req, res, next) => {
  try {
    const userId = req?.userId
    const { id } = req?.params

    const {
      company_name,
      solution_id,
      address,
      average_monthly_consumption,
      city_id,
      email,
      branch_id,
      lead_status_id,
      name,
      phone,
      pincode,
      sanctioned_load,
      source_id,
      state_id
    } = req?.body

    const existingLead = await Lead.findById(id)

    if (!existingLead) {
      return errorResponse(res, 'Leads does not exist', {}, 404)
    }

    const isConverted = lead_status_id == '69d5dfb78c890e742280d9cb'

    await Lead.findByIdAndUpdate(id, {
      company_name,
      name,
      address,
      pincode,
      email,
      phone,
      average_monthly_consumption,
      branch_id,
      city_id,
      state_id,
      lead_status_id,
      is_converted: isConverted,
      source_id: convertVar(source_id),
      solution_id: convertVar(solution_id),
      sanctioned_load,
      assigned_user_id: userId,
      updated_by: userId,
      updated_at: Date.now()
    })

    return successResponse(res, 'Leads updated successfully')
  } catch (error) {
    next(error)
  }
}

...
const putDesignationAPI = async (req, res, next) => {
  try {
    ...
    const designation = await Designation.findOne({ company_id: userIid, _id: req.params.id });
...
const deleteDesignationAPI = async (req, res, next) => {
  try {
    ...
    const designation = await Designation.findOne({ company_id: userId, _id: designationId });
